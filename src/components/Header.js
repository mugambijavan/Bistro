...
const Header = () => {
  ...
  return (
    <motion.header variants ={headerVariants} initial="hidden" animate={isActive ? 'show' : 'hidden'} className='bg-pink-200/20 fixed w-full max-w-[1800px] z-50 py-4'>
      <motion.div variants={staggerContainer(0.3, 1)} initial='hidden' animate={"show"} className='container mx-auto'>
        <div className='flex justify-between items-center px-4 lg:px-0 relative text-white'>
          ...
          <motion.div variants={navVariants} inital='hidden' animate={nav ? 'show' : "hidden"} className='absolute bg-accent w-[275px] h-[50vh] right-0 lg:left-0 top-[120px] bottom-0 z-50 rounded-lg shadow-xl'>
